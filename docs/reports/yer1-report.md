...
## Ronda visual del dueño (2026-09-01, fixes 1-4)

1. **Bloque 2**: segundo párrafo reemplazado por el texto final del dueño (añade "life transition," / "transiciones de vida").
2. **Reseñas**: (a) fondo con la técnica exacta de la home (`.reviews-bg` con REVIEW_BG.webp en ruta absoluta, verificado por estilo computado); (b) orden nuevo K Siana → Diego Atoche → Anna Vergés, los tres byte-idénticos al inventario de la home de cada idioma (Diego Atoche localizado y extraído con el extractor corregido); (c) cada card completa es un `<a>` al destino canónico de reseñas del sitio (share.google/dQE0CvnGhGx0mbjZq, el mismo href de los ratings existentes), target _blank, rel noopener, cursor pointer.
3. **Ubicación**: eyebrow Location/Ubicación + H2 + párrafo + **mapa embebido del widget oficial de Yandex** (iframe map-widget/v1 con ll=pt=44.514208,40.206453 y z=17, coordenadas obtenidas expandiendo el enlace corto del dueño, que resuelve a la ficha org 88929712346), lazy loading, 360px, bordes de card del sitio, y el enlace "Open in Yandex Maps →"/"Abrir en Yandex Maps →" conservado bajo el mapa. **Render verificado por CDP: el widget renderiza en la vista previa local (h=360) en ambos idiomas y anchos: no hizo falta el fallback.** Esto también resuelve el M5 del auditor: el enlace corto apunta a la ficha de la propia consulta en Komitas.
4. **Citabilidad**: H2 del bloque de sesiones en forma de pregunta ("How do sessions in Yerevan work?" / "¿Cómo funcionan las sesiones en Ereván?"); FAQ v2 con las seis del dueño (entra la de precio con $120/$170 canon; sale la de armenio/ruso, cuyo dato vive en el cuerpo), schema FAQPage resincronizado (visible=schema 6/6 en ambas) y anti-dup global intacto (FAQ total sigue en 214, 0 duplicadas).

Suite re-ejecutada: ALL PASS (reseñas byte-idénticas en su wrapper clicable, blockquotes 154, wa.me 469, JSON-LD 0 err, imágenes 0 rotas, h1 despejado, overflow 0).
